...
try {
    ...
    // ── SHOPS ────────────────────────────────────────────────────────────────
        ...
        if ($action === 'CREATE' || $action === 'UPDATE') {
             ...
             // CREATE must not reuse the app's local SQLite row id — it often
             // collides with an unrelated server shop and becomes an UPDATE.
             $existsById = null;
             if ($action === 'UPDATE' && $id) {
    ...
    // ── STORIES ──────────────────────────────────────────────────────────────
        ...
        if ($action === 'CREATE' || $action === 'UPDATE') {
            ...
            if ($id) {
                ...
                $stmt->execute([$id]);
                uza_safe_unlink_uploads($db, $mediaUrls, (int) $id);
                echo json_encode(['success' => true, 'id' => $id, 'action' => 'DELETE']);
                exit;
            } else {
                http_response_code(400);
